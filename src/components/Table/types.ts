...
export interface IProps<T> {
	columns: IColumns;
	fetchRowDetailData?: ({ rowId }: { rowId: string }) => Promise<any>;
	fetchTableData?: ({ page, sort }: IFetchTableDataArgs) => Promise<any>;
	loading: boolean;
	pagination: ITablePagination;
	renderRow?: () => React.ReactNode;
	renderRowDetail?: ({ columns, row }) => React.ReactNode;
	renderAttachment?: JSX.Element;
	linkComponent?: JSX.Element;
	rows: T[];
...
export type ITableBodyProps<T> = Pick<
	IProps<T>,
	| 'columns'
	| 'fetchRowDetailData'
	| 'loading'
...
export type IRowDetailProps<T> = Pick<
	ITableBodyProps<T>,
	'columns' | 'fetchRowDetailData'
> & {
	row: T;
};

export type ITableFooterProps<T> = Pick<
	IProps<T>,
	'columns' | 'fetchTableData' | 'loading' | 'pagination'
> & {
	orderBy: IColumn['key'];
	sortOrder: SortOrder;
};
