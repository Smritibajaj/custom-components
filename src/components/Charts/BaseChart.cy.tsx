...
import BaseChart from './BaseChart.stories';

const baseChartSeries: SeriesOptionsType[] = [
	{
		name: 'Active users',
		type: 'area',
		data: [
...
describe('BaseChart.cy.tsx', () => {
	it('should mount and render chart properly.', () => {
	...
	it('chart should be visible and height should be greater than 200px', () => {
		const title = 'Sample Title';
		const subTitle = 'Sample subtitle';
		mount(
			<ThemeWrapper>
	...
	it('should hide and show the chart single series by clicking on legend.', () => {
		const title = 'Sample Title';
		const subTitle = 'Sample subtitle';
		mount(
			<ThemeWrapper>
		...
		cy.get(
			'.highcharts-legend.highcharts-no-tooltip .highcharts-legend-item.highcharts-series-0'
		)
			.click()
			.wait(3000)
			.click();
