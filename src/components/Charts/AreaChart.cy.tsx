...
const areaChartSeries: SeriesAreaOptions[] = [
	{
		name: 'Active users',
		type: 'area',
		data: [
...
describe('AreaChart.cy.tsx', () => {
	it('should mount and render area chart properly.', () => {
	...
	it('should validate series data for chart.', () => {
		cy.wrap(areaChartSeries).should(data => {
			...
			expect(firstSeries).to.haveOwnProperty(
				'type',
				'area',
				'property type exist with value as area in series'
			);
			expect(data).to.be.equal(areaChartSeries);
		});
	});
});
